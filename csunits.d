...
struct Bytes(T: ulong)
{
    ...
    string toString(bool inBits = false) const @property @trusted /* pure nothrow */
    {
        ...
        if     (val < 1024^^1) {
            ...
            return to!string(cast(real)val / 1024^^1) ~ " kilo" ~ name;
        } else if (val < 1024^^3) {
            return to!string(cast(real)val / 1024^^2) ~ " Mega" ~ name;
        } else if (val < 1024^^4) {
            return to!string(cast(real)val / 1024^^3) ~ " Giga" ~ name;
        } else if (val < 1024^^5) {
            return to!string(cast(real)val / 1024^^4) ~ " Tera" ~ name;
        } else if (val < 1024^^6) {
            return to!string(cast(real)val / 1024^^5) ~ " Peta" ~ name;
        } else if (val < 1024^^7) {
            return to!string(cast(real)val / 1024^^6) ~ " Exa" ~ name;
        } else if (val < 1024^^8) {
            return to!string(cast(real)val / 1024^^7) ~ " Zetta" ~ name;
        } else /* if (val < 1024^^9) */ {
            return to!string(cast(real)val / 1024^^8) ~ " Yotta" ~ name;
        /* } else { */
        /*     return to!string(val) ~ " " ~ name; */
        }
    }
