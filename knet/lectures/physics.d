                           ...
                           Lang lang = Lang.en, Origin origin = Origin.manual)
{
    foreach (expr; File(`../knowledge/en/chemical_elements.txt`).byLine.filter!(a => !a.empty))
    {
        ...
        graph.connect(graph.add(name, lang, Sense.nounUncountable, origin),
                      Role(Rel.instanceOf),
                      graph.add(`chemical element`, lang, Sense.nounSingular, origin),
                      origin, weight);

        graph.connect(graph.add(sym, lang, Sense.noun, origin),
                      Role(Rel.symbolFor),
                      graph.add(name, lang, Sense.noun, origin),
                      origin, weight);
    }
}
