    ...
    {									\
      ...
      if (!discard_saved_regs)                                          \
        {                                                               \
          regstart[reg] = start;                                        \
          regend[reg] = end;                                            \
          DEBUG_PRINT ("     Pop reg %d (spanning %p -> %p)\n",        \
...
at_begline_loc_p (const_re_char *pattern, const_re_char *p, reg_syntax_t syntax)
{
  ...
  /* After a positive or negative lookahead/lookbehind subexpression?  */
  else if ((*prev == '=' || *prev == '!'))
    {
      /* Skip over optional lookbehind qualifier.  */
      if (prev - 1 >= pattern && prev[-1] >= '<')
	--prev;
