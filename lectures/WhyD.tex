...
\begin{frame}[fragile]{Leading Questions}
  ...
  \item Why so many languages: SAAB SimC uses C/C++, Fortran,
  ...
  % \item Get to work on what's meaningful
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Language Trend}
  ...
  \item Game Industry: ``To few good C++ developers''. Why?
  \item Which fault is it---developers or language designers?
...
\begin{frame}[fragile]{Democratic \& Pragmatic Fundaments}
    \begin{columns}[c] % the "c" option specifies center vertical alignment
      ...
      \item Gold is \emph{not} everything that\ldots --- Big leaps are
        \emph{not} made by the largest
      \item \textbf{Prestigeless Non-Company Controlled}: No more:
        \textit{``Lets reinvent everything because we want to be
          different''}
      ...
      \item Linux-style Deprecation Model
      \end{itemize}
      \column{.35\textwidth}
      \begin{figure}
...
\begin{frame}[fragile]{List of Upcoming Range Creations}
  \begin{itemize}[<+->]
  \item \textbf{\texttt{enumerate}}: (Just merged)
  \item \textbf{\texttt{cache}}: Inject caching of (in-pipeline) calculations of
    intermediate results. (Just merged)
  \item \textbf{\texttt{each}}: (PR on review)
  \item \textbf{\texttt{chunkBy}}: (PR on review)
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Safe Concurrency: Example}
\begin{lstlisting}[frame=single]
...
void main(T args)
{
    ...
    auto f3 = spawn(&useArgs, args); // errors as: "Aliases to mutable thread-local data not allowed."
}
\end{lstlisting}
...
\begin{frame}[fragile]{Safe Code}
  ...
  \item \emph{not} do unsafe \texttt{cast}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Compilers}
  \begin{itemize}[<+->]
  \item \textbf{DMD}: Reference (Recent Features), Fast Compile
  \item \textbf{GDC}: GCC, Fast Code
  \item \textbf{LDC}: LLVM (ca 1 month behing DMD), Fast Code
  \item \textbf{DDMD}: coming ``soon''.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{IDEs}
  ...
  \item \textbf{Emacs FlyCheck}: Including visual feedback overlays of unittests!
...
\end{lstlisting}
  ...
  \item CTFE is slow for complex usage such as \textbf{Pegged}.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{What's Next?}
  ...
  \item \textbf{Reference Counted String}: \texttt{rcstring}
