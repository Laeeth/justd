...
\author[Calle Isaksson \& Per Nordlöw, HiQ]{\textbf {Calle Isaksson \& Per
    Nordlöw\\HiQ}} % auteur
...
\begin{frame}[fragile]{Leading Questions}
  ...
  \item SAAB SimC uses C/C++, Fortran, Ada, Python, Shell, Make, \ldots =>
...
\begin{frame}[fragile]{The Scientist and The Craftman ---\\D's Magic Pair!}
  \begin{figure}
    \fbox{\includegraphics[width=0.95\textwidth]{andreibright}}
  \end{figure}
\end{frame}
...
\begin{frame}[fragile]{Arrays Builtin \& Done Right}
  \begin{itemize}[<+->]
  \item Easier Transition from C/C++ with: \texttt{int a[2];}
...
\begin{frame}[fragile]{Pragmatic Array Slice Semantics}
  ...
  \item Pointer part decide ``\textbf{null}ness''
    \begin{lstlisting}[frame=single]
assert([].ptr == null);
assert("ab"[$..$].ptr != null);
    \end{lstlisting}
...
assert("ab"[$..$]); // contextual hit (EOL)
  ...
  \item Empty slices implicit convert to \textbf{false}
    \begin{lstlisting}[frame=single]
assert(![]);
assert(!null);
    \end{lstlisting}
...
\begin{frame}[fragile]{Compilation Performance Facts}
  ...
  \item Compiler DMD itself builds on 20s on my Quad Core laptop.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{The Big Picture}
  ...
  \item \textbf{Unique Feature}: Deduction of purity (\texttt{pure}) and Safety
    (\texttt{@safe}) if compiler has the source!
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{More Inference than in C++11}
  Along with type inference through \texttt{auto}, D also inferences
  \begin{itemize}[<+->]
  \item Accessibility (auto, const or immutable) using inout and
  \item Value/Reference Semantics using auto ref
  \end{itemize}
  This is a natural extension of \texttt{auto} as compiler has this knowledge
  (more function-bodies are available by default in D) and reduce code bloat.
\end{frame}
...
\begin{frame}[fragile]{Easy, Flexible \& Safe Module System}
  ...
  \item No need for separate .h and .c to keep in sync by default...but
  \item Compiler can auto-generate interface files based on source code
...
\begin{frame}[fragile]{What's Next?}
  ...
  \item DMD \& Phobos each has around 100 Pending Github Pull Requests!:
    \begin{itemize}[<+->]
    \item \textbf{Multi-Dim Slicing} a la Matlab och Fortran 90+: \texttt{x[0..m, 0..n]}
...
\begin{frame}[fragile]{Conclusion: Component Programming}
  ...
  \end{figure}
  \begin{itemize}[<+->]
  \item Features come together in an elegant and concise harmony =>
