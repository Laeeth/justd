...
\item Add keyword for spawning threads/fibers: go f(x); instead of current
  spawn(&f, x); similar to Go. See: https://gobyexample.com/goroutines.
\item Perhaps
\item - sync f(x);
\item - async f(x);
...
\item ubyte[20] d; assert(d[].allZeros());
\item Stack-trace for failed unittests or debuggable
\item Better feedback for unmatched template predicates:
\item /home/per/Work/justd/geometry.d(553): Error: template instance
...
\item should instead say something like:
\item template parameter D doesn’t fullfil predicate (D >= 1)
